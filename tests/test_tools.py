...
def create_tools(
    ...
    repository = TodoRepository(tmp_path / "todos.db")
...
def test_add_and_list_todo(
    ...
    assert add_result == "已添加待办，ID=1，内容：学习 SQLite"

    assert list_result == "ID=1 | 未完成 | 学习 SQLite"
...
def test_complete_todo(
    ...
    complete_result = tools["complete_todo"].invoke(
        {
            "todo_id": 1,
        }
    )
    ...
    assert complete_result == ("已将 ID=1 的待办标记为完成。")

    assert list_result == ("ID=1 | 已完成 | 完成数据库练习")
...
def test_delete_todo_approved(
    ...
    delete_result = tools["delete_todo"].invoke(
        {
            "todo_id": 1,
        }
    )
...
def test_delete_todo_rejected(
    ...
    assert delete_result == ("用户拒绝删除该待办事项，操作已取消。")

    assert list_result == "ID=1 | 未完成 | 必须保留"
...
def test_delete_missing_todo_after_approval(
    ...
    result = tools["delete_todo"].invoke(
        {
            "todo_id": 999,
        }
    )

    assert result == ("未找到 ID=999 的待办，或该待办不属于当前用户。")
...
def test_data_survives_repository_recreation(
    ...
    first_repository = TodoRepository(database_path)
    ...
    second_repository = TodoRepository(database_path)
    ...
    assert result == ("ID=1 | 未完成 | 需要持久保存的任务")
