...
def test_record_and_list_conversations(
    ...
    repository.record_message(
        owner_id="owner-1",
        thread_id="thread-1",
        first_message=("请帮我制定一个学习LangGraph的计划"),
    )

    conversations = repository.list_conversations(owner_id="owner-1")

    assert len(conversations) == 1

    assert conversations[0].thread_id == "thread-1"

    assert conversations[0].title.startswith("请帮我制定")
...
def test_second_message_does_not_replace_title(
    ...
    assert conversation.title == "第一条消息"
...
def test_rename_conversation(
    ...
    assert conversation.title == "LangGraph学习计划"
